...
**Monitoring approach:** live LangSmith tracing of every AI-assisted interaction in the MVP (retrieval, classification, checklist summary, predictive alert — see [../mvp/mvp_documentation.md](../mvp/mvp_documentation.md), "Monitoring — LangSmith tracing"), not just the one-off Round 1 sample it grew from ([../langsmith/monitoring_notes.md](../langsmith/monitoring_notes.md)), **plus an LLM-as-judge evaluator** ([../mvp/core/judge.py](../mvp/core/judge.py), run either from the CLI [../mvp/scripts/judge_traces.py](../mvp/scripts/judge_traces.py) or from the app's own **📊 Judge Reports** page) that independently scores real traces for correctness and hallucination and posts the scores back as LangSmith feedback on each run — so accuracy isn't just observable, it's actively measured, not merely logged, and the measurement itself is visible in the same product an installer or reviewer already has open, not only in a separate LangSmith login. This sits alongside the dashboard metrics already scoped in [../dashboard/dashboard_documentation.md](../dashboard/dashboard_documentation.md) — first-visit fix rate, false-hardware-fault rate, connectivity-failure rate by model/firmware. Together these give an operational accuracy signal and a real audit trail per interaction, even though formal accuracy/robustness requirements (Art. 15) aren't mandatory at this tier.
...
7. **Performance/evaluation approach** — offline unit tests (20 tests, [../mvp/mvp_documentation.md](../mvp/mvp_documentation.md)), live per-interaction LangSmith tracing in the MVP (grown from the Round 1 trace sample/experiment — [../langsmith/monitoring_notes.md](../langsmith/monitoring_notes.md)), an LLM-as-judge evaluator scoring real traces for correctness and hallucination ([../mvp/scripts/judge_traces.py](../mvp/scripts/judge_traces.py)), dashboard operational metrics
