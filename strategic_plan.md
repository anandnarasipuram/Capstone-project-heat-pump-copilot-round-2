...
The Streamlit application in [mvp/](mvp/): real RAG (OpenAI embeddings + Pinecone), all three use-case modes functional, live LangSmith tracing of every interaction (not the POC's placeholder — see [mvp/mvp_documentation.md](mvp/mvp_documentation.md), "Monitoring"), those traces scored for correctness/hallucination by an LLM-as-judge visible in-app on the 📊 Judge Reports page, 20 offline unit tests passing, graceful degradation with zero API keys. **This phase's job is to prove the capability works before any real installer sees it** — it is deliberately internal-only (Chleo's own team, synthetic/public data), not yet a pilot.
