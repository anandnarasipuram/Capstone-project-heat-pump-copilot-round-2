...
Round 1 built the research pack, a synthetic fault dataset, a Tableau dashboard, an n8n/Telegram POC (keyword-grounded fault triage), and a LangSmith monitoring sample — see [feedback/round1_decision.md](feedback/round1_decision.md) for why the use case was kept unchanged. Round 2 deepens the same use case rather than starting over: the POC's keyword match becomes real embeddings-based RAG (OpenAI + Pinecone) in a working MVP that also builds out the two use-case candidates Round 1 named but didn't build (a commissioning-completeness checker and a COP-drop predictive early-warning tool), and adds the full consulting package — ROI/risk, EU AI Act + GDPR compliance, and a phased strategic deployment plan — that a real client would need before greenlighting a pilot.
